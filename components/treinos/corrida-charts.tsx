...
export type PeriodoPonto = { label: string; km: number; atual: boolean };

/** Km por período (mês ou ano) — mesmo visual do VolumeChart, período em destaque. */
export function PeriodoChart({ data }: { data: PeriodoPonto[] }) {
  return (
    <div className="h-[220px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
          ...
          />
          <Bar dataKey="km" radius={[6, 6, 0, 0]} maxBarSize={32} animationDuration={chart.animMs}>
            {data.map((d) => (
              <Cell key={d.label} fill={d.atual ? chart.mint : chart.surface2} />
            ))}
          </Bar>
